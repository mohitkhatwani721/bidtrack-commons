...
export const getRelevantPlaceholder = (productName: string): string => {
  ...
  if (lowerName.includes("refrigerator") || lowerName.includes("fridge")) {
    ...
    // Updated microwave image URL to a more reliable one
...
export const generateAdditionalImages = (productName: string, mainImage: string): string[] => {
  ...
  if (lowerName.includes("refrigerator") || lowerName.includes("fridge")) {
    ...
    // Added more reliable microwave oven images
    if (additionalImages.length < 3) additionalImages.push("https://images.unsplash.com/photo-1618160702438-9b02ab6515c9?q=80&w=1000&auto=format&fit=crop");
